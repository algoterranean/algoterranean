...
class Stats (MonoBehaviour):
	...
	def OnGUI():
		s = "Chunks: $chunks_visible / $chunks_max ($chunks_created total)"
		...
		p = "Performance\n"
		p += "Name" + " " * (col1_len - 4) + "Total (ms)" + " " * (col2_len-10) + "Avg (ms)" + " " * (col3_len - 9) + "Max (ms)" + " " * (col4_len-8) + "\n"
		p += "Chunk" + " " * (col1_len-5) + "$block_creation_count" + " " * (col2_len-len(block_creation_count)) + "$b_time" + " " * (col3_len-len(b_time)) + "$b_time_max" + " " * (col4_len-len(b_time_max)) + "\n"
		p += "Mesh" + " " * (col1_len-4) + "$mesh_creation_count" + " " * (col2_len-len(mesh_creation_count)) + "$m_time" + " " * (col3_len-len(m_time)) + "$m_time_max" + " " * (col4_len-len(m_time_max)) + "\n"
